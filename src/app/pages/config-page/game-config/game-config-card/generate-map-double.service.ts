export class MapDoubleGenerator {
  ...
  private generateMap(isReverse: boolean) {
    let count = 0
    for (let i = 0; i < this.countRound; i++) {
      if (isReverse) this.mapTeams.reverse()
  ...
  public generate(){
    ...
    const map1 = JSON.parse(JSON.stringify(this.map))
    const countNotTaskMap1 = this.countNotTask(map1)
    this.generateMap(false)
    const map2 = this.map
    const countNotTaskMap2 = this.countNotTask(map1)
    if (countNotTaskMap1<countNotTaskMap2){
      this.map = map1
    }
  ...
  private countNotTask(map:number[][][]):number{
    let count = 0
    for(let i of map){
      for(let j of i){
        for(let t of j){
  ...
  private  getNext(rowIndex:number, columnIndex:number) {
    ...
    return this.getNextAny(rowIndex, columnIndex)
  }
